...
public:
  ...
  Contact(RigidBody *_body0, RigidBody *_body1, const Eigen::Vector3f &_p,
          const Eigen::Vector3f &_n, float _phi);
  ...
  // and the provided direction @a dir, which is aligned with the first tangent
  // direction. The resulting frame is stored in the basis vectors @a n, @a t1,
  // and @a t2.
  //
