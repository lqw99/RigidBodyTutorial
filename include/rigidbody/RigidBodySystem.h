...
public:
  ...
  // Add rigid body @a _b to the system. The rigid body is owned and managed by
  // this RigidBodySystem.
  ...
  float contactStiffness; // Contact stiffness, used for computing Baumgarte
                          // stabilization parameters.
  float contactDamping;   // Contact damping, used for computing Baumgarte
                          // stabilization parameters.
...
private:
  ...
  PreStepFunc m_preStepFunc; // Callback function that is called before body
                             // positions and orientations are updated.
  ResetFunc m_resetFunc;     // Callback for simulation reset.
};

template <typename ScalarType, int Options>
Eigen::Quaternion<ScalarType, Options>
operator*(ScalarType a, const Eigen::Quaternion<ScalarType, Options> &q) {
  return Eigen::Quaternion<ScalarType, Options>(a * q.w(), a * q.x(), a * q.y(),
                                                a * q.z());
}

template <typename ScalarType, int Options>
Eigen::Quaternion<ScalarType, Options>
operator+(const Eigen::Quaternion<ScalarType, Options> &q1,
          const Eigen::Quaternion<ScalarType, Options> &q2) {
  return Eigen::Quaternion<ScalarType, Options>(
