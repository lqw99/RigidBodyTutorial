...
class RigidBody {
public:
  RigidBody(float _mass, Geometry *_geometry,
  ...
  Eigen::Matrix3f Ibody,
      IbodyInv;         // Inertia and inverse inertia in the local body frame.
  ...
  std::vector<Contact *>
      contacts; // Pointer array of contact constraints involving this body.

  polyscope::SurfaceMesh *mesh; // Used for rendering

  Eigen::Vector3f grav = Eigen::Vector3f(0.f, -9.81f, 0.f);
};
