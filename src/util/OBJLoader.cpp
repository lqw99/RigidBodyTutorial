...
//--------------------------------------------------------------------------------------------------
// Load file
bool OBJLoader::load(const std::string &filename, Eigen::MatrixXf &meshV,
                     Eigen::MatrixXi &meshF) {
  // Open the input file
  std::ifstream file(filename.c_str(), std::ifstream::in);
  if (!file.is_open()) {
    std::cout << "Error: Failed to open file " << filename << " for reading!"
              << std::endl;
    return false;
  }
  ...
  // Close file
  file.close();
  return true; // missing return value
}
